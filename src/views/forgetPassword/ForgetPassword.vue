  ...
  <div class="rel">
    ...
    <div class="form">
      ...
      <div class="list pwd" :class="{active:formData.password!='',show:showPwd}">
        <input :type="showPwd?'text':'password'" v-model="formData.password" placeholder="重置密码">
        <div class="r_icon" @click="showPwd=!showPwd"></div>
      </div>
      <div class="list pwd" :class="{active:formData.password2!='',show:showPwd2}">
        <input :type="showPwd2?'text':'password'" v-model="formData.password2" placeholder="再次确认密码">
        <div class="r_icon" @click="showPwd2=!showPwd2"></div>
      </div>

      <div class="submit" @click="submit" :class="{active:isRegisterBtn}">提交</div>
...
import { sendCodeApi, sendEmailCodeApi, resetPasswordApi } from "@/api";
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      resetPasswordApi(this.formData).then(data => {
        this.$router.goBack();
      });
    },
    // 验证手机号
